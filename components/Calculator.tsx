...
const calculationMemoCache = new Map<
	string,
	{ result: BigNumber; timestamp: number }
>();
...
export default function Calculator() {
	...
	const calculateResults = () => {
		...
		const newState: CalculatorState = {
			...
			additionalGems: inputs.additionalGems
				? new BigNumber(inputs.additionalGems)
				: null,
		...
		let calculationCache = {
			...
			goalTimeCalc: null as {
				timeToGoal: BigNumber;
		...
		const updateResults = () => {
			...
			const timeSinceLastFullUpdate =
				currentTime - calculationCache.lastFullUpdateTime;
			const timeSinceLastLightUpdate =
				currentTime - calculationCache.lastLightUpdateTime;

			// Determine update type with more granular control
			const needsLightUpdate =
				timeSinceLastLightUpdate >= lightUpdateInterval;
			const needsFullUpdate =
				timeSinceLastFullUpdate >= fullUpdateInterval;
			...
			if (
				calculationCache.baseCalculations.profitPerSecond &&
				currentTime -
					calculationCache.baseCalculations.lastCalculationTime <
					1000
			) {
				profitPerSecond =
					calculationCache.baseCalculations.profitPerSecond;
			} else {
				profitPerSecond = newState.currentGems
					.times(newState.hourlyInterest!)
					.dividedBy(3600);
				calculationCache.baseCalculations.profitPerSecond =
					profitPerSecond;
				calculationCache.baseCalculations.lastCalculationTime =
			...
			const profitGrowth = totalProfit
				.dividedBy(newState.currentGems)
				.times(100);
			...
			if (needsFullUpdate) {
				...
				currentProfitPerSecond = currentTotal
					.times(newState.hourlyInterest!)
					.dividedBy(3600);
				...
				if (newState.goalGems) {
					if (newState.goalGems.lte(newState.currentGems)) {
						...
						const shouldUseCache =
							calculationCache.goalTimeCalc &&
							(hasVeryLargeGems || hasExtremeGems);

						if (shouldUseCache) {
							timeToGoal =
								calculationCache.goalTimeCalc!.timeToGoal;
							...
							if (
								cachedLog &&
								Date.now() - cachedLog.timestamp < CACHE_TTL * 2
							) {
				...
				if (newState.additionalGems) {
					...
					// More aggressive caching for large numbers
					const shouldUseCache =
						calculationCache.additionalTimeCalc &&
						(hasVeryLargeGems || hasExtremeGems);
				...
				if (
					calculationCache.additionalResult &&
					newState.additionalGems
				) {
					newResults.timeToAdditional =
						calculationCache.additionalResult;
				}
			}
